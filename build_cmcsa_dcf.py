#!/usr/bin/env python3
"""
build_cmcsa_dcf.py - Institutional DCF Valuation Model for Comcast Corporation (NASDAQ: CMCSA)
...
def build_cmcsa_dcf_model(output_path="CMCSA_DCF_Model_Gemini-3.7-Flash_20260818.xlsx"):
    ...
    # ==========================================
    ws_wacc["A1"] = "Comcast Corporation (NASDAQ: CMCSA)"
    ...
    wacc_equity_inputs = [
        ("Risk-Free Rate (10-Yr US Treasury Yield)", 0.0470, "0.00%", "Source: US 10-Year Treasury Yield benchmark as of August 18, 2026", "B5"),
        ("Equity Beta (5-Year Monthly vs S&P 500)", 0.67, "0.00", "Source: Market data 5-year monthly regression beta (Reflects defensive cash flows)", "B6"),
    ...
    wacc_debt_inputs = [
        ("Pre-Tax Cost of Debt (Kd)", 0.0525, "0.00%", "Source: Form 10-K & debt schedule effective borrowing cost on outstanding senior notes", "B11"),
        ("Effective Corporate Tax Rate (t)", 0.2200, "0.0%", "Source: Form 10-K normalized federal and state marginal corporate tax rate", "B12"),
        ...
        ("Total Debt (Current + Noncurrent) ($M)", 98937.00, "$#,##0.00", "Source: Form 10-K Balance Sheet total carrying debt balance ($98.94B)", "B17"),
        ("Cash and Cash Equivalents ($M)", 9481.00, "$#,##0.00", "Source: Form 10-K Balance Sheet cash & cash equivalents ($9.48B)", "B18"),
    ...
    # --- Title Block ---
    ws_dcf["A1"] = "Comcast Corporation (NASDAQ: CMCSA)"
    ws_dcf["A1"].font = font_title
    ws_dcf["A2"] = "Institutional DCF Valuation Model | Report Date: August 18, 2026 | Financial Source: SEC Form 10-K & Consensus Estimates"
    ...
    ws_dcf["B4"].comment = Comment("Case Selector: 1 = Bear Case, 2 = Base Case (Consensus Baseline), 3 = Bull Case (Growth Acceleration)", "DCF Model Builder")
    
    ws_dcf["C4"] = '=IF(B4=1,"[1] BEAR CASE (Cord-Cutting Headwinds / Margin Pressure)",IF(B4=2,"[2] BASE CASE (Broadband ARPU / Mobile Expansion / Epic Universe)","[3] BULL CASE (Accelerated Digital & Streaming Monetization)"))'
    ...
    market_inputs_dcf = [
        ("Current Stock Price ($)", 26.19, "$#,##0.00", "Source: Market close price as of August 18, 2026", "B7"),
        ("Diluted Shares Outstanding (M)", 3550.00, "#,##0.00", "Source: SEC Form 10-K diluted share count reflecting buybacks (3,550M shares)", "B8"),
        ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
        ("Cash and Cash Equivalents ($M)", 9481.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet cash & equivalents ($9.48B)", "B10"),
        ("Total Debt (Current + Noncurrent) ($M)", 98937.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet total carrying debt ($98.94B)", "B11"),
        ("Net Debt ($M)", "=B11-B10", "$#,##0.00", None, "B12"),
        ("Implied Enterprise Value ($M)", "=B9+B12", "$#,##0.00", None, "B13"),
        ("Effective Corporate Tax Rate (%)", 0.220, "0.0%", "Source: Normalized corporate effective tax rate", "B14"),
    ...
    for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs_dcf, start=7):
        ...
        if "='WACC Build'" in str(val):
            ...
            if comment_text:
                ws_dcf[f"B{idx}"].comment = Comment(comment_text, "Comcast 10-K / Market Data")
    ...
    scenario_blocks = [
        ("Bear Case Assumptions (Selector = 1)", 22, [
            ("  Revenue Growth (%)", [-0.010, -0.005, 0.000, 0.005, 0.005], 0.010, "0.0%"),
            ("  EBIT Margin (%)", [0.160, 0.158, 0.155, 0.152, 0.150], 0.0700, "0.0%"),
            ("  D&A (% of Revenue)", [0.072, 0.072, 0.072, 0.072, 0.072], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.095, 0.092, 0.090, 0.088, 0.088], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.010, 0.010, 0.010, 0.010, 0.010], None, "0.0%"),
        ]),
        ("Base Case Assumptions (Selector = 2)", 29, [
            ("  Revenue Growth (%)", [0.015, 0.018, 0.020, 0.018, 0.015], 0.015, "0.0%"),
            ("  EBIT Margin (%)", [0.170, 0.175, 0.180, 0.182, 0.185], "='WACC Build'!E26", "0.0%"),
            ("  D&A (% of Revenue)", [0.070, 0.070, 0.070, 0.070, 0.070], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.095, 0.090, 0.088, 0.085, 0.085], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.010, 0.010, 0.010, 0.010, 0.010], None, "0.0%"),
        ]),
        ("Bull Case Assumptions (Selector = 3)", 36, [
            ("  Revenue Growth (%)", [0.030, 0.032, 0.030, 0.025, 0.020], 0.020, "0.0%"),
            ("  EBIT Margin (%)", [0.175, 0.182, 0.190, 0.195, 0.200], 0.0600, "0.0%"),
            ("  D&A (% of Revenue)", [0.068, 0.068, 0.068, 0.068, 0.068], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.090, 0.085, 0.082, 0.080, 0.080], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.010, 0.010, 0.010, 0.010, 0.010], None, "0.0%"),
        ]),
    ]
    ...
    # Axis ranges centered on Base Case (WACC = 6.28%, g = 1.50%)
    g_cols = [0.005, 0.010, 0.015, 0.020, 0.025]
    wacc_rows = [0.0528, 0.0578, 0.0628, 0.0678, 0.0728]
    ...
    ebit_cols = [0.155, 0.170, 0.185, 0.200, 0.215]
    ...
    beta_rows = [0.47, 0.57, 0.67, 0.77, 0.87]
