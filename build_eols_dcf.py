...
# --- Title Block ---
ws["A1"] = "Evolus, Inc. (NASDAQ: EOLS)"
ws["A1"].font = font_title
ws["A2"] = "Institutional DCF Valuation Model | Report Date: August 18, 2026 | Source: SEC Form 10-K & Company Disclosures"
...
market_inputs = [
    ("Current Stock Price ($)", 8.15, "$#,##0.00", "Market close price as of mid-August 2026 ($8.15)", "B7"),
    ("Diluted Shares Outstanding (M)", 65.20, "#,##0.00", "SEC Form 10-K FY2024 / Q1 2026 Diluted share count (65.20M shares)", "B8"),
    ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
    ("Cash and Cash Equivalents ($M)", 87.00, "$#,##0.00", "SEC Form 10-K Balance sheet as of Dec 31, 2024 ($87.00M)", "B10"),
    ("Total Debt ($M)", 125.00, "$#,##0.00", "SEC Form 10-K Balance sheet Pharmakon Term Loan principal ($125.00M)", "B11"),
    ...
    ("Risk-Free Rate (10-Yr US Treasury) (%)", 0.0470, "0.00%", "10-Year US Treasury Benchmark Yield (August 2026)", "B15"),
    ("Equity Beta (5-Year Monthly)", 1.36, "0.00", "5-Year Monthly Beta vs S&P 500 (1.36)", "B16"),
    ("Market Equity Risk Premium (ERP) (%)", 0.0550, "0.00%", "Standard institutional market equity risk premium (5.50%)", "B17"),
    ("Pre-Tax Cost of Debt (%)", 0.0950, "0.00%", "Pharmakon Term Loan interest rate (~9.50%)", "B18"),
]
...
for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs, start=7):
    ...
    if comment_text:
        ws[f"B{idx}"].comment = Comment(comment_text, "Evolus 10-K / Market Data")

# --- Section II: Scenario Assumptions & Driver Blocks ---
add_section_header(20, "II. SCENARIO ASSUMPTIONS & DRIVER BLOCKS")

headers_s2 = ["Driver / Assumption", "FY2024A", "FY2025E", "FY2026E", "FY2027E", "FY2028E", "FY2029E", "Terminal / WACC"]
...
scenario_blocks = [
    ("Bear Case Assumptions (Selector = 1)", 22, [
        ("  Revenue Growth (%)", [0.100, 0.085, 0.070, 0.060, 0.050], 0.020, "0.0%"),
        ("  EBIT Margin (%)", [-0.015, 0.027, 0.065, 0.097, 0.127], 0.1186, "0.0%"),
        ("  D&A (% of Revenue)", [0.015, 0.014, 0.013, 0.012, 0.011], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.012, 0.011, 0.010, 0.010, 0.009], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.015, 0.015, 0.015, 0.015, 0.015], None, "0.0%"),
    ]),
    ("Base Case Assumptions (Selector = 2)", 28, [
        ("  Revenue Growth (%)", [0.150, 0.135, 0.115, 0.095, 0.075], 0.025, "0.0%"),
        ("  EBIT Margin (%)", [0.016, 0.080, 0.134, 0.180, 0.210], 0.1186, "0.0%"),
        ("  D&A (% of Revenue)", [0.015, 0.014, 0.013, 0.012, 0.011], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.011, 0.010, 0.010, 0.009, 0.009], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.015, 0.015, 0.015, 0.015, 0.015], None, "0.0%"),
    ]),
    ("Bull Case Assumptions (Selector = 3)", 34, [
        ("  Revenue Growth (%)", [0.180, 0.160, 0.140, 0.115, 0.090], 0.030, "0.0%"),
        ("  EBIT Margin (%)", [0.040, 0.117, 0.175, 0.232, 0.263], 0.1186, "0.0%"),
        ("  D&A (% of Revenue)", [0.015, 0.014, 0.013, 0.012, 0.011], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.010, 0.010, 0.009, 0.009, 0.008], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.015, 0.015, 0.015, 0.015, 0.015], None, "0.0%"),
    ]),
]
...
for title, sub_hdr_row, rows in scenario_blocks:
    ...
    for r_offset, (label, vals, term_val, num_fmt) in enumerate(rows, start=1):
        ...
        # Col B (Historical FY2024A base reference)
...
wacc_items = [
    ...
    ("Pre-Tax Cost of Debt (Kd)", "=$B$18", "0.00%", "Row 18: Pharmakon Term Loan interest rate", False),
    ...
    ("Net Debt ($M)", "=$B$12", "$#,##0.00", "Row 12: Total Debt - Cash", False),
    ("Enterprise Value (EV) ($M)", "=$B$13", "$#,##0.00", "Row 13: Market Cap + Net Debt", False),
    ("Weight of Equity (We)", "=B56/B58", "0.0%", "Equity Value / Enterprise Value", False),
    ("Weight of Debt (Wd)", "=B57/B58", "0.0%", "Net Debt / Enterprise Value", False),
...
headers_s4 = ["Line Item ($M)", "FY2024A", "FY2025E", "FY2026E", "FY2027E", "FY2028E", "FY2029E"]
...
ws["B65"].comment = Comment("FY2024 Actual Net Revenue YoY Growth (+31.8%)", "SEC Form 10-K")
for c_idx in range(3, 8):
    col_let = get_column_letter(c_idx)
...
# Row 66: Net Revenue ($M)
ws["A66"] = "Net Revenue ($M)"
...
ws["B66"].comment = Comment("FY2024 Actual Net Revenue ($266.27M)", "SEC Form 10-K")
...
ws["B67"].number_format = "-0.0%;0.0%;0.0%"
...
# Row 68: Operating Income / (Loss) (EBIT)
ws["A68"] = "Operating Income / (Loss) (EBIT) ($M)"
ws["A68"].font = font_bold
ws["A68"].border = border_cell
ws["B68"] = -34.40
ws["B68"].font = font_bold
ws["B68"].number_format = "$#,##0.00;($#,##0.00);$0.00"
ws["B68"].alignment = align_right
ws["B68"].border = border_cell
ws["B68"].comment = Comment("FY2024 Actual Operating Loss ($(34.40)M)", "SEC Form 10-K")
...
ws["B69"] = 0.00
ws["B69"].font = font_regular
ws["B69"].number_format = "$#,##0.00;($#,##0.00);$0.00"
ws["B69"].alignment = align_right
ws["B69"].border = border_cell
for c_idx in range(3, 8):
    col_let = get_column_letter(c_idx)
    cell = ws.cell(row=69, column=c_idx, value=f"=MAX(0, {col_let}68*$B$14)")
    cell.font = font_regular
    cell.number_format = "($#,##0.00);($#,##0.00);$0.00"
    cell.alignment = align_right
    cell.border = border_cell
...
ws["B70"] = -34.40
ws["B70"].font = font_bold
ws["B70"].number_format = "$#,##0.00;($#,##0.00);$0.00"
ws["B70"].alignment = align_right
...
ws["B71"].comment = Comment("FY2024 Actual D&A (~$4.00M)", "SEC Form 10-K")
...
ws["B72"].comment = Comment("FY2024 Actual CapEx (~$2.80M)", "SEC Form 10-K")
...
ws["B73"].comment = Comment("FY2024 Working Capital increase (~$1.20M)", "SEC Form 10-K")
...
ws["A74"].font = Font(name="Calibri", size=10, bold=True, color="1F4E79")
ws["A74"].fill = fill_accent_blue
ws["A74"].border = border_total
ws["B74"] = -34.40
ws["B74"].font = Font(name="Calibri", size=10, bold=True, color="1F4E79")
ws["B74"].alignment = align_right
ws["B74"].number_format = "$#,##0.00;($#,##0.00);$0.00"
...
headers_s5 = ["Discount Parameters", "", "Year 1 (FY25E)", "Year 2 (FY26E)", "Year 3 (FY27E)", "Year 4 (FY28E)", "Year 5 (FY29E)"]
...
val_summary_items = [
    ...
    ("  Final Year Projected UFCF (FY2029E) ($M)", "=G74", "$#,##0.00", None, False),
    ...
    ("  Terminal Value at FY2029E ($M)", "=B88/(B87-B86)", "$#,##0.00", None, False),
    ("  PV of Terminal Value ($M)", "=B89/(1+B87)^4.5", "$#,##0.00", None, False),
...
t1_wacc_values = [0.1086, 0.1136, 0.1186, 0.1236, 0.1286]
...
for row_idx, w_val in enumerate(t1_wacc_values, start=109):
    ...
    for col_idx in range(2, 7):
        ...
        formula = (
            ...
            f"($G$74*(1+{col_let}$108)/($A{row_idx}-{col_let}$108))/(1+$A{row_idx})^4.5) - $B$12)/$B$8"
        )
...
# Table 2: FY25-29 Revenue Growth Delta vs Target FY29 EBIT Margin
ws["A116"] = "Sensitivity Table 2: Implied Share Price ($) vs. Revenue Growth Delta & Target FY29 EBIT Margin"
ws["A116"].font = font_subhdr
ws.merge_cells("A116:H116")

ws["A117"] = "EBIT Margin \\ Rev Growth Δ"
ws["A117"].font = font_tbl_hdr
ws["A117"].fill = fill_soft_blue
ws["A117"].alignment = align_center
ws["A117"].border = border_header

t2_rev_delta = [-0.040, -0.020, 0.000, 0.020, 0.040]
t2_ebit_margins = [0.150, 0.180, 0.210, 0.240, 0.270]

for col_idx, g_val in enumerate(t2_rev_delta, start=2):
    c = ws.cell(row=117, column=col_idx, value=g_val)
    c.font = font_tbl_hdr
    c.fill = fill_accent_blue if col_idx == 4 else fill_soft_blue
...
for row_idx, m_val in enumerate(t2_ebit_margins, start=118):
    ...
    for col_idx in range(2, 7):
        col_let = get_column_letter(col_idx)
        # Scaled DCF directly with live Excel formula (recomputed relative to base 21.0%):
        formula = (
            f"=((($C$74*(1+{col_let}$117)*($A{row_idx}/0.210))/(1+$B$61)^0.5 + "
            f"($D$74*(1+{col_let}$117)*($A{row_idx}/0.210))/(1+$B$61)^1.5 + "
            f"($E$74*(1+{col_let}$117)*($A{row_idx}/0.210))/(1+$B$61)^2.5 + "
            f"($F$74*(1+{col_let}$117)*($A{row_idx}/0.210))/(1+$B$61)^3.5 + "
            f"($G$74*(1+{col_let}$117)*($A{row_idx}/0.210))/(1+$B$61)^4.5 + "
            f"(($G$74*(1+{col_let}$117)*($A{row_idx}/0.210)*(1+$H$41)/($B$61-$H$41))/(1+$B$61)^4.5)) - $B$12)/$B$8"
        )
        cell = ws.cell(row=row_idx, column=col_idx, value=formula)
...
ws["A125"] = "Sensitivity Table 3: Implied Share Price ($) vs. Beta & 10-Year Treasury Yield (Risk-Free Rate)"
...
t3_rf_values = [0.0370, 0.0420, 0.0470, 0.0520, 0.0570]
t3_beta_values = [1.16, 1.26, 1.36, 1.46, 1.56]
...
for row_idx, beta_val in enumerate(t3_beta_values, start=127):
    ...
    for col_idx in range(2, 7):
        col_let = get_column_letter(col_idx)
        # Derived WACC = (Ke * We) + (Kd_after * Wd) = ({col_let}$126 + $A{row_idx} * 0.055) * $B$59 + $B$55 * $B$60
        wacc_expr = f"(({col_let}$126 + $A{row_idx}*0.055)*$B$59 + $B$55*$B$60)"
        formula = (
            f"=(($C$74/(1+{wacc_expr})^0.5 + $D$74/(1+{wacc_expr})^1.5 + $E$74/(1+{wacc_expr})^2.5 + "
            f"$F$74/(1+{wacc_expr})^3.5 + $G$74/(1+{wacc_expr})^4.5 + "
            f"($G$74*(1+$H$41)/({wacc_expr}-$H$41))/(1+{wacc_expr})^4.5) - $B$12)/$B$8"
        )
        cell = ws.cell(row=row_idx, column=col_idx, value=formula)
...
output_filename = "EOLS_DCF_Model_Gemini-3.7-Flash_20260818.xlsx"
