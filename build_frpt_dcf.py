...
# --- Title Block ---
ws["A1"] = "Freshpet, Inc. (NASDAQ: FRPT)"
ws["A1"].font = font_title
ws["A2"] = "Institutional DCF Valuation Model | Report Date: August 18, 2026 | Financial Source: SEC Form 10-K & Company Disclosures"
...
market_inputs = [
    ("Current Stock Price ($)", 72.76, "$#,##0.00", "Market close price as of August 2026", "B7"),
    ("Diluted Shares Outstanding (M)", 56.04, "#,##0.00", "SEC Form 10-K FY2025 Diluted share count (56.04M shares)", "B8"),
    ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
    ("Cash and Cash Equivalents ($M)", 278.00, "$#,##0.00", "SEC Form 10-K Balance sheet as of Dec 31, 2025", "B10"),
    ("Total Debt ($M)", 397.30, "$#,##0.00", "SEC Form 10-K Balance sheet (Convertible senior notes)", "B11"),
    ...
    ("Risk-Free Rate (10-Yr US Treasury) (%)", 0.0425, "0.00%", "10-Year US Treasury Benchmark Yield", "B15"),
    ("Equity Beta (5-Year Monthly)", 1.35, "0.00", "5-Year Monthly Beta vs S&P 500", "B16"),
    ("Market Equity Risk Premium (ERP) (%)", 0.0550, "0.00%", "Standard institutional market risk premium", "B17"),
    ("Pre-Tax Cost of Debt (%)", 0.0450, "0.00%", "Estimated weighted average coupon on convertible notes", "B18"),
]
...
for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs, start=7):
    ...
    if comment_text:
        ws[f"B{idx}"].comment = Comment(comment_text, "Freshpet 10-K / Market Data")
...
scenario_blocks = [
    ("Bear Case Assumptions (Selector = 1)", 22, [
        ("  Revenue Growth (%)", [0.090, 0.075, 0.060, 0.050, 0.040], 0.020, "0.0%"),
        ("  EBIT Margin (%)", [0.075, 0.085, 0.090, 0.095, 0.100], 0.115, "0.0%"),
        ("  D&A (% of Revenue)", [0.080, 0.075, 0.070, 0.065, 0.060], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.120, 0.100, 0.080, 0.060, 0.050], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
    ]),
    ("Base Case Assumptions (Selector = 2)", 28, [
        ("  Revenue Growth (%)", [0.120, 0.110, 0.095, 0.080, 0.065], 0.025, "0.0%"),
        ("  EBIT Margin (%)", [0.085, 0.100, 0.115, 0.125, 0.135], 0.115, "0.0%"),
        ("  D&A (% of Revenue)", [0.080, 0.075, 0.070, 0.065, 0.060], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.120, 0.100, 0.080, 0.060, 0.050], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
    ]),
    ("Bull Case Assumptions (Selector = 3)", 34, [
        ("  Revenue Growth (%)", [0.150, 0.140, 0.125, 0.110, 0.090], 0.030, "0.0%"),
        ("  EBIT Margin (%)", [0.095, 0.115, 0.135, 0.150, 0.160], 0.115, "0.0%"),
        ("  D&A (% of Revenue)", [0.080, 0.075, 0.070, 0.065, 0.060], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.120, 0.100, 0.080, 0.060, 0.050], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
    ]),
...
ws["B65"].comment = Comment("FY2025 Actual Net Sales YoY Growth (+13.0%)", "SEC Form 10-K")
for c_idx in range(3, 8):
    col_let = get_column_letter(c_idx)
...
ws["B66"].comment = Comment("FY2025 Actual Net Sales ($1,102.0M)", "SEC Form 10-K")
...
ws["B68"].comment = Comment("FY2025 Actual Operating Income ($75.7M)", "SEC Form 10-K")
...
ws["B71"].comment = Comment("FY2025 Actual D&A ($89.7M)", "SEC Form 10-K")
...
ws["B72"].comment = Comment("FY2025 Actual CapEx ($148.2M)", "SEC Form 10-K")
...
ws["B73"].comment = Comment("FY2025 Actual Working Capital change ($4.5M)", "SEC Form 10-K")
...
t1_wacc_values = [0.105, 0.110, 0.115, 0.120, 0.125]
...
t2_ebit_margins = [0.115, 0.125, 0.135, 0.145, 0.155]
...
for row_idx, m_val in enumerate(t2_ebit_margins, start=118):
    ...
    for col_idx in range(2, 7):
        ...
        formula = (
            f"=((($C$74*(1+{col_let}$117)*($A{row_idx}/0.135))/(1+$B$61)^0.5 + "
            f"($D$74*(1+{col_let}$117)*($A{row_idx}/0.135))/(1+$B$61)^1.5 + "
            f"($E$74*(1+{col_let}$117)*($A{row_idx}/0.135))/(1+$B$61)^2.5 + "
            f"($F$74*(1+{col_let}$117)*($A{row_idx}/0.135))/(1+$B$61)^3.5 + "
            f"($G$74*(1+{col_let}$117)*($A{row_idx}/0.135))/(1+$B$61)^4.5 + "
            f"(($G$74*(1+{col_let}$117)*($A{row_idx}/0.135)*(1+$H$41)/($B$61-$H$41))/(1+$B$61)^4.5)) - $B$12)/$B$8"
        )
        cell = ws.cell(row=row_idx, column=col_idx, value=formula)
...
t3_beta_values = [1.15, 1.25, 1.35, 1.45, 1.55]
...
output_filename = "FRPT_DCF_Model_Gemini-3.7-Flash_20260818.xlsx"
