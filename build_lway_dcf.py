...
# --- Title Block ---
ws["A1"] = "Lifeway Foods, Inc. (NASDAQ: LWAY)"
...
market_inputs = [
    ("Current Stock Price ($)", 25.82, "$#,##0.00", "Market close price as of mid-August 2026", "B7"),
    ("Diluted Shares Outstanding (M)", 15.23, "#,##0.00", "SEC Form 10-K FY2025 Diluted share count (15.23M shares)", "B8"),
    ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
    ("Cash and Cash Equivalents ($M)", 5.57, "$#,##0.00", "SEC Form 10-K Balance sheet as of Dec 31, 2025 ($5.57M)", "B10"),
    ("Total Debt ($M)", 0.00, "$#,##0.00", "SEC Form 10-K Balance sheet ($0 outstanding long-term debt)", "B11"),
    ("Net Debt ($M)", "=B11-B10", "$#,##0.00", None, "B12"),
    ("Implied Enterprise Value ($M)", "=B9+B12", "$#,##0.00", None, "B13"),
    ("Effective Corporate Tax Rate (%)", 0.250, "0.0%", "Historical effective tax rate (~25.0%)", "B14"),
    ("Risk-Free Rate (10-Yr US Treasury) (%)", 0.0470, "0.00%", "10-Year US Treasury Benchmark Yield (August 2026)", "B15"),
    ("Equity Beta (5-Year Monthly)", 0.80, "0.00", "5-Year Monthly Beta adjusted vs Consumer Staples peers", "B16"),
    ("Market Equity Risk Premium (ERP) (%)", 0.0550, "0.00%", "Standard institutional market risk premium", "B17"),
    ("Pre-Tax Cost of Debt (%)", 0.0550, "0.00%", "Estimated revolving credit facility interest rate (SOFR + spread)", "B18"),
]
...
for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs, start=7):
    ...
    if comment_text:
        ws[f"B{idx}"].comment = Comment(comment_text, "Lifeway Foods 10-K / Market Data")
...
scenario_blocks = [
    ("Bear Case Assumptions (Selector = 1)", 22, [
        ("  Revenue Growth (%)", [0.070, 0.055, 0.045, 0.035, 0.025], 0.020, "0.0%"),
        ("  EBIT Margin (%)", [0.070, 0.068, 0.065, 0.062, 0.060], 0.091, "0.0%"),
        ("  D&A (% of Revenue)", [0.020, 0.019, 0.018, 0.018, 0.018], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.040, 0.038, 0.035, 0.033, 0.030], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.030, 0.030, 0.030, 0.030, 0.030], None, "0.0%"),
    ]),
    ("Base Case Assumptions (Selector = 2)", 28, [
        ("  Revenue Growth (%)", [0.120, 0.100, 0.085, 0.070, 0.055], 0.025, "0.0%"),
        ("  EBIT Margin (%)", [0.080, 0.085, 0.090, 0.095, 0.100], 0.091, "0.0%"),
        ("  D&A (% of Revenue)", [0.020, 0.019, 0.018, 0.018, 0.018], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.038, 0.035, 0.034, 0.032, 0.030], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.030, 0.030, 0.030, 0.030, 0.030], None, "0.0%"),
    ]),
    ("Bull Case Assumptions (Selector = 3)", 34, [
        ("  Revenue Growth (%)", [0.150, 0.130, 0.110, 0.090, 0.075], 0.030, "0.0%"),
        ("  EBIT Margin (%)", [0.085, 0.095, 0.105, 0.115, 0.120], 0.091, "0.0%"),
        ("  D&A (% of Revenue)", [0.020, 0.019, 0.018, 0.018, 0.018], None, "0.0%"),
        ("  CapEx (% of Revenue)", [0.035, 0.033, 0.032, 0.030, 0.028], None, "0.0%"),
        ("  Δ NWC (% of Δ Revenue)", [0.030, 0.030, 0.030, 0.030, 0.030], None, "0.0%"),
    ]),
]
...
wacc_items = [
    ...
    ("Pre-Tax Cost of Debt (Kd)", "=$B$18", "0.00%", "Row 18: Revolving credit facility borrowing rate", False),
...
ws["B65"].comment = Comment("FY2025 Actual Net Sales YoY Growth (+13.7%)", "SEC Form 10-K")
for c_idx in range(3, 8):
    col_let = get_column_letter(c_idx)
...
ws["B66"].comment = Comment("FY2025 Actual Net Sales ($212.50M)", "SEC Form 10-K")
...
ws["B68"].comment = Comment("FY2025 Actual Operating Income ($16.20M)", "SEC Form 10-K")
...
ws["B71"].comment = Comment("FY2025 Actual D&A (~$3.60M)", "SEC Form 10-K")
...
ws["B72"].comment = Comment("FY2025 Actual CapEx ($27.4M including facility expansion)", "SEC Form 10-K")
...
ws["B73"].comment = Comment("FY2025 Actual Working Capital change (~$0.80M)", "SEC Form 10-K")
...
val_summary_items = [
    ...
    ("  Final Year Projected UFCF (FY2030E) ($M)", "=G74", "$#,##0.00", None, False),
    ...
    ("  Terminal Value at FY2030E ($M)", "=B88/(B87-B86)", "$#,##0.00", None, False),
...
t1_wacc_values = [0.081, 0.086, 0.091, 0.096, 0.101]
...
# Table 2: FY26-30 Revenue Growth Step vs Target FY30 EBIT Margin
ws["A116"] = "Sensitivity Table 2: Implied Share Price ($) vs. Revenue Growth Delta & Target FY30 EBIT Margin"
...
t2_rev_delta = [-0.020, -0.010, 0.000, 0.010, 0.020]
t2_ebit_margins = [0.080, 0.090, 0.100, 0.110, 0.120]
...
for row_idx, m_val in enumerate(t2_ebit_margins, start=118):
    ...
    for col_idx in range(2, 7):
        ...
        formula = (
            f"=((($C$74*(1+{col_let}$117)*($A{row_idx}/0.100))/(1+$B$61)^0.5 + "
            f"($D$74*(1+{col_let}$117)*($A{row_idx}/0.100))/(1+$B$61)^1.5 + "
            f"($E$74*(1+{col_let}$117)*($A{row_idx}/0.100))/(1+$B$61)^2.5 + "
            f"($F$74*(1+{col_let}$117)*($A{row_idx}/0.100))/(1+$B$61)^3.5 + "
            f"($G$74*(1+{col_let}$117)*($A{row_idx}/0.100))/(1+$B$61)^4.5 + "
            f"(($G$74*(1+{col_let}$117)*($A{row_idx}/0.100)*(1+$H$41)/($B$61-$H$41))/(1+$B$61)^4.5)) - $B$12)/$B$8"
        )
        cell = ws.cell(row=row_idx, column=col_idx, value=formula)
...
t3_beta_values = [0.60, 0.70, 0.80, 0.90, 1.00]
...
output_filename = "LWAY_DCF_Model_Gemini-3.7-Flash_20260818.xlsx"
