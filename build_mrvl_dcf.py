#!/usr/bin/env python3
"""
build_mrvl_dcf.py - Institutional DCF Valuation Model for Marvell Technology, Inc. (NASDAQ: MRVL)
...
def build_mrvl_dcf_model(output_path="MRVL_DCF_Model_Gemini-3.7-Flash_20260820.xlsx"):
    ...
    # ==========================================
    ws_wacc["A1"] = "Marvell Technology, Inc. (NASDAQ: MRVL)"
    ...
    wacc_equity_inputs = [
        ("Risk-Free Rate (10-Yr US Treasury Yield)", 0.0465, "0.00%", "Source: US 10-Year Treasury Yield benchmark as of August 20, 2026", "B5"),
        ("Equity Beta (5-Year Monthly vs S&P 500)", 2.24, "0.00", "Source: Market data 5-year monthly regression beta vs S&P 500 (Reflects semiconductor/AI volatility)", "B6"),
    ...
    wacc_debt_inputs = [
        ("Credit Rating", "Baa2 / BBB", "@", "Source: Moody's / S&P institutional investment grade credit rating profile", "B11"),
        ("Pre-Tax Cost of Debt (Kd)", 0.0520, "0.00%", "Source: Form 10-K & senior notes yield curve benchmark", "B12"),
        ("Effective Corporate Tax Rate (t)", 0.2100, "0.0%", "Source: US statutory corporate tax rate benchmark (21.0%)", "B13"),
        ...
        ("Total Debt (Short-Term + Long-Term Debt) ($M)", 4961.30, "$#,##0.00", "Source: Form 10-Q / 10-K Balance Sheet total carrying debt balance ($4.96B)", "B18"),
        ("Cash, Cash Equivalents & ST Investments ($M)", 3843.60, "$#,##0.00", "Source: Form 10-Q / 10-K Balance Sheet total cash & liquid marketable securities ($3.84B)", "B19"),
    ...
    wacc_sched = [
        ("Common Equity", "=B17/B21", "=B8", "=B25*C25", "Pure equity market weight (~99.5% of total capital structure)"),
        ("Net Debt", "=B20/B21", "=B14", "=B26*C26", "Net debt weight (~0.5% of capital structure)"),
    ]
    ...
    ws_wacc["E27"] = "Baseline market-implied discount rate (Normalized WACC benchmark = 11.50%)"
    ...
    # ==========================================
    ws_dcf["A1"] = "Marvell Technology, Inc. (NASDAQ: MRVL)"
    ...
    ws_dcf["B5"] = '=IF(B4=1,"Bear Case (Downside / Hyperscaler CapEx Pause)",IF(B4=2,"Base Case (Consensus / Custom ASIC & Electro-Optics Ramp)","Bull Case (AI Supercycle & Multi-Hyperscaler Custom Wins)"))'
    ...
    mkt_data = [
        ("Current Stock Price ($)", 237.27, "$#,##0.00", "Source: NASDAQ closing price as of August 20, 2026", "B8"),
        ("Diluted Shares Outstanding (M)", 893.30, "#,##0.00", "Source: Form 10-Q / 10-K Diluted common shares outstanding", "B9"),
        ("Market Capitalization ($M)", "=B8*B9", "$#,##0.00", None, "B10"),
        ("Cash, Cash Equivalents & ST Investments ($M)", 3843.60, "$#,##0.00", "Source: Form 10-Q / 10-K Balance Sheet total cash & liquid marketable securities ($3.84B)", "B11"),
        ("Total Debt (Short-Term + Long-Term Debt) ($M)", 4961.30, "$#,##0.00", "Source: Form 10-Q / 10-K Balance Sheet total principal debt balance ($4.96B)", "B12"),
        ("Net Debt / (Net Cash) ($M)", "=B12-B11", "$#,##0.00", None, "B13"),
        ("Normalized Corporate Tax Rate (%)", 0.2100, "0.0%", "Source: US statutory corporate tax rate benchmark (21.0%)", "B14"),
    ]
    ...
    # 1. Bear Case
    add_section_header(ws_dcf, 17, "BEAR CASE ASSUMPTIONS (Case 1 - Hyperscaler CapEx Digestion & Custom ASIC Delays)", "I", fill_color=fill_med_blue, font_color=font_sub_sec)
    ...
    bear_rows = [
        ("Revenue Growth (%)", [0.250, 0.200, 0.150, 0.100, 0.070, None], "0.0%", "Hyperscalers delay custom ASIC rollouts; optical DSP growth normalizes"),
        ("Gross Margin (%)", [0.480, 0.485, 0.490, 0.495, 0.500, None], "0.0%", "Competitive pressure in optical DSP and higher wafer costs"),
        ("EBIT Margin (%)", [0.180, 0.200, 0.220, 0.230, 0.240, None], "0.0%", "OpEx deleverage as revenue growth slows relative to high R&D commitment"),
        ("CapEx % Revenue", [0.048, 0.045, 0.042, 0.040, 0.038, 0.035], "0.0%", "Continued mask set and advanced packaging testing expenditures"),
        ("D&A % Revenue", [0.120, 0.100, 0.085, 0.075, 0.070, 0.065], "0.0%", "Amortization of acquired intangible assets and lab equipment depreciation"),
        ("NWC Change % ΔRev", [0.025, 0.025, 0.025, 0.025, 0.025, None], "0.0%", "Working capital requirement from inventory buildup"),
        ("Terminal Growth Rate (%)", [None, None, None, None, None, 0.0250], "0.00%", "Conservative long-term GDP growth rate (2.50%)"),
        ("WACC (Discount Rate) (%)", [None, None, None, None, None, 0.1300], "0.00%", "Higher equity risk premium & cyclical semiconductor volatility (13.00%)"),
    ]
    ...
    # 2. Base Case
    add_section_header(ws_dcf, 28, "BASE CASE ASSUMPTIONS (Case 2 - Consensus Custom Silicon & Optical DSP Expansion)", "I", fill_color=fill_med_blue, font_color=font_sub_sec)
    ...
    base_rows = [
        ("Revenue Growth (%)", [0.403, 0.435, 0.300, 0.200, 0.150, None], "0.0%", "Consensus & company targets: Custom ASIC (Google/AWS) + 800G/1.6T optics ramp"),
        ("Gross Margin (%)", [0.535, 0.550, 0.560, 0.565, 0.570, None], "0.0%", "Product mix shifts to high-margin electro-optics and high-volume custom compute"),
        ("EBIT Margin (%)", [0.240, 0.280, 0.300, 0.320, 0.330, None], "0.0%", "Significant operating leverage as revenue scales past $15B-$25B"),
        ("CapEx % Revenue", [0.045, 0.042, 0.040, 0.038, 0.035, 0.035], "0.0%", "Disciplined capital expenditure aligned with fabless semiconductor business model"),
        ("D&A % Revenue", [0.120, 0.095, 0.080, 0.070, 0.065, 0.065], "0.0%", "D&A scales down as % of revenue as legacy acquisition intangibles run off"),
        ("NWC Change % ΔRev", [0.020, 0.020, 0.020, 0.020, 0.020, None], "0.0%", "Normalized working capital requirements on incremental revenue"),
        ("Terminal Growth Rate (%)", [None, None, None, None, None, 0.0300], "0.00%", "Global technological GDP expansion rate (3.00%)"),
        ("WACC (Discount Rate) (%)", [None, None, None, None, None, 0.1150], "0.00%", "Institutional normalized WACC factoring AI data center leadership (11.50%)"),
    ]
    ...
    # 3. Bull Case
    add_section_header(ws_dcf, 39, "BULL CASE ASSUMPTIONS (Case 3 - AI Data Center Supercycle, Multi-Tier ASIC Dominance & CXL)", "I", fill_color=fill_med_blue, font_color=font_sub_sec)
    ...
    bull_rows = [
        ("Revenue Growth (%)", [0.480, 0.500, 0.350, 0.250, 0.180, None], "0.0%", "AI data center connectivity booms; custom ASIC wins expand to 4+ hyperscalers"),
        ("Gross Margin (%)", [0.550, 0.570, 0.580, 0.585, 0.590, None], "0.0%", "High-density 1.6T/3.2T optical DSP and custom AI XPUs command premium margins"),
        ("EBIT Margin (%)", [0.260, 0.300, 0.330, 0.350, 0.360, None], "0.0%", "Exceptional operating leverage with revenue outpacing SG&A/R&D growth"),
        ("CapEx % Revenue", [0.042, 0.040, 0.038, 0.035, 0.032, 0.032], "0.0%", "Efficient capital intensity leveraging foundry partners (TSMC 3nm/2nm)"),
        ("D&A % Revenue", [0.120, 0.090, 0.075, 0.065, 0.060, 0.060], "0.0%", "Optimized amortization schedule as revenue expands rapidly"),
        ("NWC Change % ΔRev", [0.015, 0.015, 0.015, 0.015, 0.015, None], "0.0%", "Strong bargaining power and advance customer prepayments"),
        ("Terminal Growth Rate (%)", [None, None, None, None, None, 0.0350], "0.00%", "Secular AI connectivity and custom compute market leadership (3.50%)"),
        ("WACC (Discount Rate) (%)", [None, None, None, None, None, 0.1050], "0.00%", "Lower equity risk premium reflecting entrenched hyperscaler design wins (10.50%)"),
    ]
    ...
    ws_dcf["B63"] = 5508.00
    ws_dcf["B63"].comment = Comment("Source: Form 10-K FY2024 Consolidated Statements of Operations ($5,508M)", "DCF Model Builder")
    ws_dcf["C63"] = 5767.00
    ws_dcf["C63"].comment = Comment("Source: Form 10-K FY2025 Consolidated Statements of Operations ($5,767M)", "DCF Model Builder")
    ws_dcf["D63"] = 8195.00
    ws_dcf["D63"].comment = Comment("Source: Form 10-K FY2026 Consolidated Statements of Operations ($8,195M)", "DCF Model Builder")
    ...
    ws_dcf["B64"] = -0.0696
    ws_dcf["B64"].comment = Comment("Source: Form 10-K FY2024 Revenue YoY Growth (-7.0%)", "DCF Model Builder")
    ...
    ws_dcf["B65"] = 3214.00
    ws_dcf["B65"].comment = Comment("Source: Form 10-K FY2024 Cost of Goods Sold", "DCF Model Builder")
    ws_dcf["C65"] = 3385.00
    ws_dcf["C65"].comment = Comment("Source: Form 10-K FY2025 Cost of Goods Sold", "DCF Model Builder")
    ws_dcf["D65"] = 4014.00
    ws_dcf["D65"].comment = Comment("Source: Form 10-K FY2026 Cost of Goods Sold", "DCF Model Builder")
    ...
    for col_c in ["E", "F", "G", "H", "I"]:
        ws_dcf[f"{col_c}68"] = f"={col_c}63*0.175"
    ...
    ws_dcf["B73"] = -92.00
    ws_dcf["B73"].comment = Comment("Source: Form 10-K FY2024 Income Tax Benefit/Provision", "DCF Model Builder")
    ws_dcf["C73"] = -110.00
    ws_dcf["C73"].comment = Comment("Source: Form 10-K FY2025 Income Tax Benefit/Provision", "DCF Model Builder")
    ws_dcf["D73"] = 277.83
    ws_dcf["D73"].comment = Comment("Source: Form 10-K FY2026 Effective Tax Provision on positive EBIT", "DCF Model Builder")
    for col_c in ["E", "F", "G", "H", "I"]:
    ...
    bridge_rows = [
        ...
        ("Net Debt / (Net Cash) ($M)", "=B101-B102", "$#,##0.00", font_bold, "Net debt subtracted from Enterprise Value", "B103"),
        ("IMPLIED EQUITY VALUE ($M)", "=B100-B103", "$#,##0.00", font_bold_navy, "Equity Value = Enterprise Value - Net Debt", "B104"),
        ("Diluted Shares Outstanding (M)", "=B9", "#,##0.00", font_regular, "Diluted shares outstanding from Form 10-Q / 10-K", "B105"),
        ("IMPLIED DCF VALUE PER SHARE ($)", "=B104/B105", "$#,##0.00", font_bold_navy, "Intrinsic value per share derived from DCF valuation", "B106"),
        ("Current Market Share Price ($)", "=B8", "$#,##0.00", font_regular, "Market price as of August 20, 2026", "B107"),
    ...
    ebit_margin_axis = [0.290, 0.310, 0.330, 0.350, 0.370] # Base = 33.0% (Col E)
    rev_growth_axis = [0.320, 0.360, 0.403, 0.440, 0.480]  # Base = 40.3% (Row 126)
    
    for c_i, m_val in enumerate(ebit_margin_axis, start=3):
        col_l = get_column_letter(c_i)
        cell = ws_dcf[f"{col_l}123"]
        cell.value = m_val
        cell.font = font_input_bold if m_val == 0.330 else font_input
        cell.fill = fill_accent_blue if m_val == 0.330 else fill_soft_blue
        cell.number_format = "0.0%"
        cell.alignment = align_center
        cell.border = border_header
    ...
    for r_i, rev_g in enumerate(rev_growth_axis, start=124):
        ...
        cell_lbl.font = font_input_bold if rev_g == 0.403 else font_input
        cell_lbl.fill = fill_accent_blue if rev_g == 0.403 else fill_soft_blue
    ...
    rf_axis = [0.0415, 0.0440, 0.0465, 0.0490, 0.0515] # Base = 4.65% (Col E)
    beta_axis = [1.80, 2.00, 2.24, 2.45, 2.65]         # Base = 2.24 (Row 136)
    
    for c_i, rf_val in enumerate(rf_axis, start=3):
        col_l = get_column_letter(c_i)
        cell = ws_dcf[f"{col_l}133"]
        cell.value = rf_val
        cell.font = font_input_bold if rf_val == 0.0465 else font_input
        cell.fill = fill_accent_blue if rf_val == 0.0465 else fill_soft_blue
        cell.number_format = "0.00%"
        cell.alignment = align_center
        cell.border = border_header
    ...
    for r_i, beta_val in enumerate(beta_axis, start=134):
        ...
        cell_lbl.font = font_input_bold if beta_val == 2.24 else font_input
        cell_lbl.fill = fill_accent_blue if beta_val == 2.24 else fill_soft_blue
        cell_lbl.number_format = "0.00"
        cell_lbl.alignment = align_center
        cell_lbl.border = border_cell
        ...
        for c_i, rf_val in enumerate(rf_axis, start=3):
            ...
            cell = ws_dcf[f"{col_l}{r_i}"]
            fml = (
                f"=((E91/(1+({col_l}$133+$B{r_i}*0.055-0.0547))^E89 + F91/(1+({col_l}$133+$B{r_i}*0.055-0.0547))^F89 + G91/(1+({col_l}$133+$B{r_i}*0.055-0.0547))^G89 + H91/(1+({col_l}$133+$B{r_i}*0.055-0.0547))^H89 + I91/(1+({col_l}$133+$B{r_i}*0.055-0.0547))^I89) "
                f"+ (I91*(1+$H$58)/(({col_l}$133+$B{r_i}*0.055-0.0547)-$H$58))/(1+({col_l}$133+$B{r_i}*0.055-0.0547))^I89 - $B$13) / $B$9"
            )
            cell.value = fml
            cell.number_format = "$#,##0.00"
            cell.alignment = align_right
