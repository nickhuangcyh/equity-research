...
def build_msft_dcf_model(output_path="MSFT_DCF_Model_Gemini-3.7-Flash_20260819.xlsx"):
    ...
    # Color Palette definition (Institutional Classic Navy & Slate Blue)
    ...
    def add_section_header(ws, row, title, max_col="K"):
        ws[f"A{row}"] = title
    ...
    # =========================================================================
    # SHEET 2: WACC Build (Cost of Capital Schedule)
    # =========================================================================
    ws_wacc["A1"] = "Microsoft Corporation (NASDAQ: MSFT)"
    ...
    wacc_equity_inputs = [
        ("Risk-Free Rate (10-Yr US Treasury Yield)", 0.0475, "0.00%", "US 10-Year Treasury Yield benchmark as of August 19, 2026", "B5"),
        ("Equity Beta (5-Year Monthly vs S&P 500)", 1.10, "0.00", "5-Year Monthly Equity Beta vs S&P 500 index (Institutional Market Benchmark)", "B6"),
    ...
    wacc_debt_inputs = [
        ("Pre-Tax Cost of Debt (Kd)", 0.0495, "0.00%", "Weighted average yield on senior unsecured debt; AAA credit rating (~20 bps spread over 10Y UST)", "B11"),
        ("Effective Corporate Tax Rate (t)", 0.2000, "0.0%", "Effective corporate income tax rate per SEC Form 10-K FY2026", "B12"),
        ...
        ("Total Debt ($M)", 47600.00, "$#,##0.00", "Total short-term and long-term debt per SEC Form 10-K ended June 30, 2026", "B17"),
        ("Cash and Short-Term Investments ($M)", 76843.00, "$#,##0.00", "Cash, cash equivalents & short-term investments as of June 30, 2026 Form 10-K", "B18"),
        ("Net Debt ($M) [Net Cash if negative]", "=B17-B18", "$#,##0.00", None, "B19"),
        ("Total Capitalization ($M)", "=B16+B17", "$#,##0.00", None, "B20"),
    ]
    ...
    # --- Title Block ---
    ws_dcf["A1"] = "Microsoft Corporation (NASDAQ: MSFT)"
    ws_dcf["A1"].font = font_title
    ws_dcf["A2"] = "Institutional DCF Valuation Model | Report Date: August 19, 2026 | Financial Source: SEC Form 10-K (FY2022-FY2026)"
    ...
    ws_dcf["B4"].comment = Comment("Case Selector: 1 = Bear Case (AI Digestion / Slowdown), 2 = Base Case (Consensus AI Cloud Scaling), 3 = Bull Case (AI Supercycle)", "DCF Model Builder")
    
    ws_dcf["C4"] = '=IF(B4=1,"[1] BEAR CASE (AI Digestion / Cloud Slowdown)",IF(B4=2,"[2] BASE CASE (Consensus AI Cloud Scaling & Copilot Monetization)","[3] BULL CASE (AI Supercycle & Azure Platform Dominance)"))'
    ...
    add_section_header(ws_dcf, 6, "I. MARKET DATA & CAPITAL STRUCTURE INPUTS", "K")
    
    market_inputs_dcf = [
        ("Current Stock Price ($)", 481.59, "$#,##0.00", "Market close price as of August 19, 2026", "B7"),
        ("Diluted Shares Outstanding (M)", 7435.00, "#,##0.00", "SEC Form 10-K FY2026 Diluted share count as of June 30, 2026 (7,435M shares)", "B8"),
        ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
        ("Cash and Short-Term Investments ($M)", 76843.00, "$#,##0.00", "SEC Form 10-K Balance Sheet as of June 30, 2026 ($20,935M cash + $55,908M short-term investments)", "B10"),
        ("Total Debt ($M)", 47600.00, "$#,##0.00", "SEC Form 10-K Balance Sheet as of June 30, 2026 (Long-term debt + short-term debt)", "B11"),
        ("Net Debt ($M) [Net Cash if negative]", "=B11-B10", "$#,##0.00", None, "B12"),
        ("Implied Enterprise Value ($M)", "=B9+B12", "$#,##0.00", None, "B13"),
        ("Effective Corporate Tax Rate (%)", 0.200, "0.0%", "Effective corporate tax rate from SEC Form 10-K FY2026", "B14"),
    ...
    for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs_dcf, start=7):
        ...
        if "='WACC Build'" in str(val):
            ...
            if comment_text:
                ws_dcf[f"B{idx}"].comment = Comment(comment_text, "MSFT 10-K / Market Data")
                
    # --- Section II: Scenario Assumptions & Driver Blocks ---
    add_section_header(ws_dcf, 20, "II. SCENARIO ASSUMPTIONS & DRIVER BLOCKS", "K")
    ...
    scenario_blocks = [
        ("Bear Case Assumptions (Selector = 1)", 22, [
            ("  Revenue Growth (%)", [0.110, 0.095, 0.085, 0.075, 0.065], 0.0250, "0.0%"),
            ("  Gross Margin (%)", [0.675, 0.675, 0.670, 0.665, 0.660], None, "0.0%"),
            ("  EBIT Margin (%)", [0.455, 0.450, 0.445, 0.440, 0.435], 0.1150, "0.0%"),
            ("  D&A (% of Revenue)", [0.135, 0.145, 0.140, 0.135, 0.125], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.300, 0.260, 0.220, 0.180, 0.150], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.015, 0.015, 0.015, 0.015, 0.015], None, "0.0%"),
        ]),
        ("Base Case Assumptions (Selector = 2)", 29, [
            ("  Revenue Growth (%)", [0.145, 0.130, 0.115, 0.100, 0.085], 0.0300, "0.0%"),
            ("  Gross Margin (%)", [0.685, 0.690, 0.695, 0.700, 0.702], None, "0.0%"),
            ("  EBIT Margin (%)", [0.470, 0.478, 0.486, 0.493, 0.500], "='WACC Build'!E26", "0.0%"),
            ("  D&A (% of Revenue)", [0.130, 0.140, 0.135, 0.130, 0.120], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.280, 0.230, 0.190, 0.160, 0.135], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.010, 0.010, 0.010, 0.010, 0.010], None, "0.0%"),
        ]),
        ("Bull Case Assumptions (Selector = 3)", 36, [
            ("  Revenue Growth (%)", [0.175, 0.160, 0.140, 0.125, 0.105], 0.0350, "0.0%"),
            ("  Gross Margin (%)", [0.695, 0.705, 0.712, 0.718, 0.725], None, "0.0%"),
            ("  EBIT Margin (%)", [0.480, 0.492, 0.505, 0.515, 0.525], 0.1000, "0.0%"),
            ("  D&A (% of Revenue)", [0.125, 0.135, 0.130, 0.125, 0.115], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.260, 0.210, 0.175, 0.145, 0.120], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.010, 0.010, 0.010, 0.010, 0.010], None, "0.0%"),
        ]),
    ]
    ...
    # --- Section III: Historical & Projected Income Statement (Rows 51-64) ---
    add_section_header(ws_dcf, 51, "III. FINANCIAL PERFORMANCE & 5-YEAR PROJECTIONS ($M)", "K")
    
    headers_is = ["Income Statement Line Item ($M)", "FY2022A", "FY2023A", "FY2024A", "FY2025A", "FY2026A", "FY2027E", "FY2028E", "FY2029E", "FY2030E", "FY2031E"]
    for col_idx, text in enumerate(headers_is, start=1):
        c = ws_dcf.cell(row=52, column=col_idx, value=text)
        c.font = font_tbl_hdr
        c.fill = fill_soft_blue
        c.alignment = align_center if col_idx > 1 else align_left
        c.border = border_header
    ...
    ws_dcf["E53"] = 281724.0
    ws_dcf["F53"] = 331839.0
    ws_dcf["G53"] = "=F53*(1+C44)"
    ws_dcf["H53"] = "=G53*(1+D44)"
    ws_dcf["I53"] = "=H53*(1+E44)"
    ws_dcf["J53"] = "=I53*(1+F44)"
    ws_dcf["K53"] = "=J53*(1+G44)"
    
    # Row 54: Revenue YoY Growth (%)
    ws_dcf["A54"] = "  Revenue YoY Growth (%)"
    ws_dcf["B54"] = "-"
    ...
    for col_idx in range(2, 12):
        c_let = get_column_letter(col_idx)
        ws_dcf[f"{c_let}56"] = f"={c_let}55/{c_let}53"
        
    # Row 57: Research & Development (R&D)
    ws_dcf["A57"] = "  Research and Development (R&D)"
    ws_dcf["B57"] = 24512.0
    ws_dcf["C57"] = 27195.0
    ws_dcf["D57"] = 29510.0
    ws_dcf["E57"] = 32488.0
    ws_dcf["F57"] = 37830.0
    ws_dcf["G57"] = "=G53*0.112"
    ws_dcf["H57"] = "=H53*0.110"
    ws_dcf["I57"] = "=I53*0.108"
    ws_dcf["J57"] = "=J53*0.105"
    ws_dcf["K57"] = "=K53*0.102"
    
    # Row 58: Sales & Marketing (S&M)
    ws_dcf["A58"] = "  Sales and Marketing (S&M)"
    ws_dcf["B58"] = 21825.0
    ws_dcf["C58"] = 22759.0
    ws_dcf["D58"] = 24260.0
    ws_dcf["E58"] = 25654.0
    ws_dcf["F58"] = 27870.0
    ws_dcf["G58"] = "=G53*0.082"
    ws_dcf["H58"] = "=H53*0.078"
    ws_dcf["I58"] = "=I53*0.075"
    ws_dcf["J58"] = "=J53*0.072"
    ws_dcf["K58"] = "=K53*0.070"
    
    # Row 59: General & Administrative (G&A)
    ws_dcf["A59"] = "  General and Administrative (G&A)"
    ws_dcf["B59"] = 5414.0
    ws_dcf["C59"] = 7575.0
    ws_dcf["D59"] = 7807.0
    ws_dcf["E59"] = 7256.0
    ws_dcf["F59"] = 4528.0
    ws_dcf["G59"] = "=G55-G60-G57-G58"  # Derived to tie out exactly to Operating Income (EBIT)
    ws_dcf["H59"] = "=H55-H60-H57-H58"
    ws_dcf["I59"] = "=I55-I60-I57-I58"
    ws_dcf["J59"] = "=J55-J60-J57-J58"
    ws_dcf["K59"] = "=K55-K60-K57-K58"
    ...
    for col_idx in range(2, 12):
        c_let = get_column_letter(col_idx)
        ws_dcf[f"{c_let}61"] = f"={c_let}60/{c_let}53"
        
    # Row 62: Effective Tax Provision
    ws_dcf["A62"] = "(-) Provision for Income Taxes"
    ws_dcf["B62"] = 10978.0
    ws_dcf["C62"] = 16950.0
    ws_dcf["D62"] = 21080.0
    ws_dcf["E62"] = 25700.0
    ...
    for col_idx in range(2, 12):
        c_let = get_column_letter(col_idx)
        ws_dcf[f"{c_let}63"] = f"={c_let}60-{c_let}62"
        
    for r in range(53, 64):
        is_bold_row = r in [53, 55, 60, 63]
        ws_dcf[f"A{r}"].font = font_bold if is_bold_row else font_regular
        ws_dcf[f"A{r}"].border = border_total if r in [53, 60, 63] else border_cell
        
        for col_idx in range(2, 12):
            c_let = get_column_letter(col_idx)
            cell = ws_dcf[f"{c_let}{r}"]
            cell.border = border_total if r in [53, 60, 63] else border_cell
            ...
            if str(cell.value).startswith("="):
                ...
                cell.comment = Comment("Source: SEC Form 10-K FY2022-FY2026 Historical Financials", "DCF Model Builder")
                
    # --- Section IV: Free Cash Flow Schedule (Rows 65-73) ---
    add_section_header(ws_dcf, 65, "IV. UNLEVERED FREE CASH FLOW (UFCF) BUILD ($M)", "K")
    
    headers_fcf = ["Cash Flow Line Item ($M)", "FY2022A", "FY2023A", "FY2024A", "FY2025A", "FY2026A", "FY2027E", "FY2028E", "FY2029E", "FY2030E", "FY2031E"]
    ...
    ws_dcf["E70"] = 4000.0
    ws_dcf["F70"] = 5000.0
    ws_dcf["G70"] = "=(G53-F53)*C49"
    ws_dcf["H70"] = "=(H53-G53)*D49"
    ws_dcf["I70"] = "=(I53-H53)*E49"
    ws_dcf["J70"] = "=(J53-I53)*F49"
    ws_dcf["K70"] = "=(K53-J53)*G49"
    ...
    for r in range(67, 73):
        ...
        for col_idx in range(2, 12):
            ...
            if str(cell.value).startswith("="):
                ...
                cell.font = font_input_bold if is_bold_row else font_input
                cell.fill = fill_input
                cell.comment = Comment("Source: SEC Form 10-K FY2022-FY2026 Cash Flows Statements", "DCF Model Builder")
                
            if r == 71:
                cell.fill = fill_accent_blue
                
    # --- Section V: Discounting Schedule & Valuation Summary (Rows 74-96) ---
    add_section_header(ws_dcf, 74, "V. DISCOUNTING SCHEDULE & VALUATION SUMMARY ($M)", "K")
    ...
    # Base Case centered (WACC = 10.71%, g = 3.00%)
    g_cols = ["=$H$44-0.01", "=$H$44-0.005", "=$H$44", "=$H$44+0.005", "=$H$44+0.01"]
    wacc_rows = ["=$H$46-0.01", "=$H$46-0.005", "=$H$46", "=$H$46+0.005", "=$H$46+0.01"]
    ...
    for c_idx, g_val in enumerate(g_cols, start=2):
        ...
        ws_dcf[f"{c_let}101"].number_format = "0.00%"
    ...
    ws_dcf["A111"] = "Growth Multiplier \\ EBIT Margin"
    ...
    ebit_cols = ["=$G$46-0.04", "=$G$46-0.02", "=$G$46", "=$G$46+0.02", "=$G$46+0.04"]
    ...
    rf_cols = ["='WACC Build'!$B$5-0.01", "='WACC Build'!$B$5-0.005", "='WACC Build'!$B$5", "='WACC Build'!$B$5+0.005", "='WACC Build'!$B$5+0.01"]
    beta_rows = ["='WACC Build'!$B$6-0.20", "='WACC Build'!$B$6-0.10", "='WACC Build'!$B$6", "='WACC Build'!$B$6+0.10", "='WACC Build'!$B$6+0.20"]
    ...
    ws_dcf.column_dimensions["K"].width = 16
    
    wb.save(output_path)
    print(f"Model saved successfully to {output_path}")

if __name__ == "__main__":
    build_msft_dcf_model()
