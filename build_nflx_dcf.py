#!/usr/bin/env python3
"""
build_nflx_dcf.py - Institutional DCF Valuation Model for Netflix, Inc. (NASDAQ: NFLX)
...
def build_nflx_dcf_model(output_path="NFLX_DCF_Model_Gemini-3.7-Flash_20260819.xlsx"):
    ...
    # ==========================================
    ws_wacc["A1"] = "Netflix, Inc. (NASDAQ: NFLX)"
    ...
    wacc_equity_inputs = [
        ("Risk-Free Rate (10-Yr US Treasury Yield)", 0.0470, "0.00%", "Source: US 10-Year Treasury Yield benchmark as of August 19, 2026 (4.70%)", "B5"),
        ("Equity Beta (5-Year Monthly vs S&P 500)", 1.25, "0.00", "Source: Market data 5-year monthly regression beta (Reflects streaming leadership & high growth)", "B6"),
    ...
    wacc_debt_inputs = [
        ("Pre-Tax Cost of Debt (Kd)", 0.0525, "0.00%", "Source: Form 10-K & debt schedule effective coupon rate on outstanding senior notes", "B11"),
        ("Effective Corporate Tax Rate (t)", 0.1600, "0.0%", "Source: Form 10-K normalized effective income tax rate (~16.0%)", "B12"),
        ...
        ("Total Debt (Current + Noncurrent) ($M)", 14463.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet total carrying debt / senior notes ($14.46B)", "B17"),
        ("Cash and Cash Equivalents & Short-Term Investments ($M)", 9068.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet cash & short-term investments ($9.07B)", "B18"),
    ...
    # --- Title Block ---
    ws_dcf["A1"] = "Netflix, Inc. (NASDAQ: NFLX)"
    ws_dcf["A1"].font = font_title
    ws_dcf["A2"] = "Institutional DCF Valuation Model | Report Date: August 19, 2026 | Financial Source: SEC Form 10-K & Consensus Outlook"
    ...
    ws_dcf["B4"].comment = Comment("Case Selector: 1 = Bear Case, 2 = Base Case (Consensus Baseline), 3 = Bull Case (High Monetization)", "DCF Model Builder")
    
    ws_dcf["C4"] = '=IF(B4=1,"[1] BEAR CASE (Ad Growth Slowdown / Content Cost Pressure)",IF(B4=2,"[2] BASE CASE (Ad-Tier Doubling / Margin Expansion to 34.5%)","[3] BULL CASE (Global ARPU Expansion / High Operating Leverage)"))'
    ...
    market_inputs_dcf = [
        ("Current Stock Price ($)", 77.77, "$#,##0.00", "Source: Market close price as of August 18, 2026 (Reflects 10-for-1 stock split in Nov 2025)", "B7"),
        ("Diluted Shares Outstanding (M)", 4260.00, "#,##0.00", "Source: SEC Form 10-K / 10-Q diluted share count reflecting buybacks (4,260.0M split-adjusted)", "B8"),
        ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
        ("Cash, Cash Equivalents & Short-Term Investments ($M)", 9068.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet cash & short-term investments ($9.07B)", "B10"),
        ("Total Debt (Senior Notes & Borrowings) ($M)", 14463.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet total carrying debt ($14.46B)", "B11"),
        ("Net Debt ($M)", "=B11-B10", "$#,##0.00", None, "B12"),
        ("Implied Enterprise Value ($M)", "=B9+B12", "$#,##0.00", None, "B13"),
        ("Effective Corporate Tax Rate (%)", 0.160, "0.0%", "Source: Normalized corporate effective tax rate reflecting FDII & stock comp deductions", "B14"),
    ...
    for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs_dcf, start=7):
        ...
        if "='WACC Build'" in str(val):
            ...
            if comment_text:
                ws_dcf[f"B{idx}"].comment = Comment(comment_text, "Netflix 10-K / Market Data")
    ...
    scenario_blocks = [
        ("Bear Case Assumptions (Selector = 1)", 22, [
            ("  Revenue Growth (%)", [0.100, 0.085, 0.075, 0.065, 0.055], 0.020, "0.0%"),
            ("  EBIT Margin (%)", [0.300, 0.302, 0.305, 0.308, 0.310], 0.1200, "0.0%"),
            ("  D&A (% of Revenue)", [0.009, 0.009, 0.009, 0.009, 0.009], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.016, 0.016, 0.016, 0.016, 0.016], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
        ]),
        ("Base Case Assumptions (Selector = 2)", 29, [
            ("  Revenue Growth (%)", [0.133, 0.120, 0.110, 0.095, 0.080], 0.030, "0.0%"),
            ("  EBIT Margin (%)", [0.315, 0.325, 0.335, 0.340, 0.345], "='WACC Build'!E26", "0.0%"),
            ("  D&A (% of Revenue)", [0.009, 0.009, 0.009, 0.009, 0.009], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.015, 0.015, 0.015, 0.015, 0.015], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
        ]),
        ("Bull Case Assumptions (Selector = 3)", 36, [
            ("  Revenue Growth (%)", [0.150, 0.140, 0.130, 0.115, 0.100], 0.035, "0.0%"),
            ("  EBIT Margin (%)", [0.325, 0.338, 0.350, 0.358, 0.365], 0.1050, "0.0%"),
            ("  D&A (% of Revenue)", [0.009, 0.009, 0.009, 0.009, 0.009], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.014, 0.014, 0.014, 0.014, 0.014], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
        ]),
    ]
    ...
    # Axis ranges centered on Base Case (WACC = 11.46%, g = 3.00%)
    # Let's calculate base WACC ~ 11.46% (0.1146). Center row = 0.1146, step = 0.005 (0.50%)
