#!/usr/bin/env python3
"""
build_ssp_dcf.py - Institutional DCF Valuation Model for The E.W. Scripps Company (NASDAQ: SSP)
...
def build_ssp_dcf_model(output_path="SSP_DCF_Model_Gemini-3.7-Flash_20260819.xlsx"):
    ...
    # ==========================================
    ws_wacc["A1"] = "The E.W. Scripps Company (NASDAQ: SSP)"
    ...
    wacc_equity_inputs = [
        ("Risk-Free Rate (10-Yr US Treasury Yield)", 0.0435, "0.00%", "Source: US 10-Year Treasury Yield benchmark as of August 19, 2026", "B5"),
        ("Equity Beta (5-Year Monthly vs S&P 500)", 1.95, "0.00", "Source: Market data 5-year monthly regression beta (Reflects broadcast cyclicality & high leverage)", "B6"),
    ...
    wacc_debt_inputs = [
        ("Pre-Tax Cost of Debt (Kd)", 0.0780, "0.00%", "Source: Form 10-K & debt schedule effective borrowing cost on outstanding senior notes & term loans", "B11"),
        ("Effective Corporate Tax Rate (t)", 0.2500, "0.0%", "Source: Form 10-K normalized federal and state marginal corporate tax rate", "B12"),
        ...
        ("Total Debt (Current + Noncurrent) ($M)", 2660.00, "$#,##0.00", "Source: Form 10-K / 10-Q Balance Sheet total carrying debt balance ($2.66B)", "B17"),
        ("Cash and Cash Equivalents ($M)", 54.67, "$#,##0.00", "Source: Form 10-K / 10-Q Balance Sheet cash & cash equivalents ($54.67M)", "B18"),
    ...
    # --- Title Block ---
    ws_dcf["A1"] = "The E.W. Scripps Company (NASDAQ: SSP)"
    ws_dcf["A1"].font = font_title
    ws_dcf["A2"] = "Institutional DCF Valuation Model | Report Date: August 19, 2026 | Financial Source: SEC Form 10-K/10-Q & Normalized Financials"
    ...
    ws_dcf["B4"].comment = Comment("Case Selector: 1 = Bear Case (Distressed/Cord-Cutting), 2 = Base Case (Normalized Political Cycle), 3 = Bull Case (Peak Political & CTV Expansion)", "DCF Model Builder")
    
    ws_dcf["C4"] = '=IF(B4=1,"[1] BEAR CASE (Accelerated Cord-Cutting / Margin Compression / Distressed Leverage)",IF(B4=2,"[2] BASE CASE (Normalized Political Cycle / Sports & CTV Expansion / Transformation Plan)","[3] BULL CASE (Record Political Ad Surge / Accelerated CTV Growth / Peak EBITDA Expansion)"))'
    ...
    market_inputs_dcf = [
        ("Current Stock Price ($)", 3.35, "$#,##0.00", "Source: Market close price as of August 19, 2026", "B7"),
        ("Diluted Shares Outstanding (M)", 87.50, "#,##0.00", "Source: SEC Form 10-Q/10-K diluted share count (87.50M shares)", "B8"),
        ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
        ("Cash and Cash Equivalents ($M)", 54.67, "$#,##0.00", "Source: SEC Form 10-Q Balance Sheet cash & equivalents ($54.67M)", "B10"),
        ("Total Debt (Current + Noncurrent) ($M)", 2660.00, "$#,##0.00", "Source: SEC Form 10-Q Balance Sheet total carrying debt ($2,660.00M)", "B11"),
        ("Net Debt ($M)", "=B11-B10", "$#,##0.00", None, "B12"),
        ("Implied Enterprise Value ($M)", "=B9+B12", "$#,##0.00", None, "B13"),
        ("Effective Corporate Tax Rate (%)", 0.250, "0.0%", "Source: Normalized corporate effective marginal tax rate", "B14"),
    ...
    for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs_dcf, start=7):
        ...
        if "='WACC Build'" in str(val):
            ...
            if comment_text:
                ws_dcf[f"B{idx}"].comment = Comment(comment_text, "Scripps 10-K / Market Data")
    ...
    scenario_blocks = [
        ("Bear Case Assumptions (Selector = 1)", 22, [
            ("  Revenue Growth (%)", [0.040, -0.100, 0.060, -0.080, 0.030], 0.000, "0.0%"),
            ("  EBIT Margin (%)", [0.095, 0.065, 0.100, 0.060, 0.080], 0.0800, "0.0%"),
            ("  D&A (% of Revenue)", [0.065, 0.065, 0.065, 0.065, 0.065], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.022, 0.022, 0.022, 0.022, 0.022], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.005, 0.005, 0.005, 0.005, 0.005], None, "0.0%"),
        ]),
        ("Base Case Assumptions (Selector = 2)", 29, [
            ("  Revenue Growth (%)", [0.090, -0.060, 0.110, -0.050, 0.070], 0.010, "0.0%"),
            ("  EBIT Margin (%)", [0.125, 0.090, 0.145, 0.100, 0.120], "='WACC Build'!E26", "0.0%"),
            ("  D&A (% of Revenue)", [0.060, 0.060, 0.058, 0.060, 0.058], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.020, 0.020, 0.020, 0.020, 0.020], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.005, 0.005, 0.005, 0.005, 0.005], None, "0.0%"),
        ]),
        ("Bull Case Assumptions (Selector = 3)", 36, [
            ("  Revenue Growth (%)", [0.140, -0.020, 0.150, -0.020, 0.100], 0.020, "0.0%"),
            ("  EBIT Margin (%)", [0.150, 0.115, 0.175, 0.130, 0.155], 0.0650, "0.0%"),
            ("  D&A (% of Revenue)", [0.058, 0.058, 0.055, 0.058, 0.055], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.018, 0.018, 0.018, 0.018, 0.018], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.005, 0.005, 0.005, 0.005, 0.005], None, "0.0%"),
        ]),
    ]
    ...
    equity_bridge = [
        ...
        ("IMPLIED INTRINSIC VALUE PER SHARE ($)", "=MAX(0,B85/B86)", "$#,##0.00", "B87", True),
    ...
    # =========================================================================
    # SECTION VI: THREE INSTITUTIONAL 5X5 SENSITIVITY MATRICES (Rows 92-132)
    # =========================================================================
    ...
    # Axis ranges centered on Base Case (Base WACC = 6.78% / 7.00%, g = 1.00%)
    g_cols = [0.000, 0.005, 0.010, 0.015, 0.020]
    wacc_rows = [0.0578, 0.0628, 0.0678, 0.0728, 0.0778]
    ...
    for r_idx, w_val in enumerate(wacc_rows, start=96):
        ...
        for c_idx, g_val in enumerate(g_cols, start=2):
            c_let = get_column_letter(c_idx)
            formula_t1 = (
                f"=MAX(0,(($B$71/(1+$A{r_idx})^0.5 + $C$71/(1+$A{r_idx})^1.5 + $D$71/(1+$A{r_idx})^2.5 + $E$71/(1+$A{r_idx})^3.5 + $F$71/(1+$A{r_idx})^4.5) + "
    ...
    for r_idx, gr_val in enumerate(growth_rows, start=106):
        ...
        for c_idx, em_val in enumerate(ebit_cols, start=2):
            c_let = get_column_letter(c_idx)
            formula_t2 = (
                f"=MAX(0,((($B$71*$A{r_idx}*({c_let}$105/$G$45)/(1+$H$45)^0.5 + "
    ...
    for r_idx, b_val in enumerate(beta_rows, start=116):
        ...
        for c_idx, rf_val in enumerate(rf_cols, start=2):
            ...
            formula_t3 = (
                f"=MAX(0,((($B$71/(1+({wacc_formula_snippet}))^0.5 + "
