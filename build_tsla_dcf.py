#!/usr/bin/env python3
"""
build_tsla_dcf.py - Institutional DCF Valuation Model for Tesla, Inc. (NASDAQ: TSLA)
...
def build_tsla_dcf_model(output_path="TSLA_DCF_Model_Gemini-3.7-Flash_20260819.xlsx"):
    ...
    # ==========================================
    ws_wacc["A1"] = "Tesla, Inc. (NASDAQ: TSLA)"
    ...
    wacc_equity_inputs = [
        ("Risk-Free Rate (10-Yr US Treasury Yield)", 0.0465, "0.00%", "Source: US 10-Year Treasury Yield benchmark as of August 19, 2026", "B5"),
        ("Equity Beta (5-Year Monthly vs S&P 500)", 1.82, "0.00", "Source: Market data 5-year monthly regression beta reflecting growth & AI volatility", "B6"),
    ...
    wacc_debt_inputs = [
        ("Pre-Tax Cost of Debt (Kd)", 0.0550, "0.00%", "Source: SEC Form 10-K & senior notes effective borrowing rate estimate", "B11"),
        ("Effective Corporate Tax Rate (t)", 0.2100, "0.0%", "Source: Form 10-K normalized statutory federal and state marginal corporate tax rate", "B12"),
        ...
        ("Total Debt (Current + Noncurrent + Leases) ($M)", 8376.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet total carrying debt balance ($8.38B)", "B17"),
        ("Cash, Cash Equivalents & ST Investments ($M)", 44059.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet cash & short-term investments ($44.06B)", "B18"),
    ...
    # --- Title Block ---
    ws_dcf["A1"] = "Tesla, Inc. (NASDAQ: TSLA)"
    ws_dcf["A1"].font = font_title
    ws_dcf["A2"] = "Institutional DCF Valuation Model | Report Date: August 19, 2026 | Financial Source: SEC Form 10-K & Consensus Estimates"
    ...
    ws_dcf["B4"].comment = Comment("Case Selector: 1 = Bear Case (EV Saturation / FSD Delay), 2 = Base Case (Next-Gen Platform + Energy Scaling), 3 = Bull Case (Robotaxi & Optimus Commercialization)", "DCF Model Builder")
    
    ws_dcf["C4"] = '=IF(B4=1,"[1] BEAR CASE (EV Saturation / Margin Pressure / FSD Delay)",IF(B4=2,"[2] BASE CASE (Next-Gen Platform / Megapack Scale / Steady FSD)","[3] BULL CASE (Autonomous Robotaxi Fleet & Optimus Surge)"))'
    ...
    market_inputs_dcf = [
        ("Current Stock Price ($)", 336.50, "$#,##0.00", "Source: Market close price as of August 19, 2026", "B7"),
        ("Diluted Shares Outstanding (M)", 3760.00, "#,##0.00", "Source: SEC Form 10-K diluted share count reflecting stock compensation (3,760M shares)", "B8"),
        ("Implied Equity Market Capitalization ($M)", "=B7*B8", "$#,##0.00", None, "B9"),
        ("Cash, Cash Equivalents & ST Investments ($M)", 44059.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet cash, cash equivalents & marketable securities ($44.06B)", "B10"),
        ("Total Debt (Current + Noncurrent + Leases) ($M)", 8376.00, "$#,##0.00", "Source: SEC Form 10-K Balance Sheet total carrying debt balance ($8.38B)", "B11"),
        ("Net Debt ($M)", "=B11-B10", "$#,##0.00", None, "B12"),
        ("Implied Enterprise Value ($M)", "=B9+B12", "$#,##0.00", None, "B13"),
        ("Effective Corporate Tax Rate (%)", 0.210, "0.0%", "Source: Normalized corporate effective tax rate", "B14"),
    ...
    for idx, (label, val, num_fmt, comment_text, cell_ref) in enumerate(market_inputs_dcf, start=7):
        ...
        if "='WACC Build'" in str(val):
            ...
            if comment_text:
                ws_dcf[f"B{idx}"].comment = Comment(comment_text, "Tesla 10-K / Market Data")
    ...
    scenario_blocks = [
        ("Bear Case Assumptions (Selector = 1)", 22, [
            ("  Revenue Growth (%)", [0.050, 0.060, 0.070, 0.060, 0.050], 0.025, "0.0%"),
            ("  EBIT Margin (%)", [0.050, 0.060, 0.070, 0.075, 0.080], 0.1550, "0.0%"),
            ("  D&A (% of Revenue)", [0.065, 0.063, 0.060, 0.058, 0.055], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.090, 0.088, 0.085, 0.082, 0.080], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.030, 0.030, 0.030, 0.030, 0.030], None, "0.0%"),
        ]),
        ("Base Case Assumptions (Selector = 2)", 29, [
            ("  Revenue Growth (%)", [0.110, 0.150, 0.160, 0.140, 0.120], 0.030, "0.0%"),
            ("  EBIT Margin (%)", [0.060, 0.080, 0.105, 0.122, 0.138], "='WACC Build'!E26", "0.0%"),
            ("  D&A (% of Revenue)", [0.058, 0.055, 0.052, 0.050, 0.048], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.085, 0.080, 0.075, 0.070, 0.065], None, "0.0%"),
            ("  Δ NWC (% of Δ Revenue)", [0.025, 0.025, 0.025, 0.025, 0.025], None, "0.0%"),
        ]),
        ("Bull Case Assumptions (Selector = 3)", 36, [
            ("  Revenue Growth (%)", [0.180, 0.250, 0.280, 0.240, 0.200], 0.040, "0.0%"),
            ("  EBIT Margin (%)", [0.085, 0.130, 0.175, 0.205, 0.230], 0.1350, "0.0%"),
            ("  D&A (% of Revenue)", [0.055, 0.052, 0.050, 0.048, 0.045], None, "0.0%"),
            ("  CapEx (% of Revenue)", [0.080, 0.075, 0.070, 0.065, 0.060], None, "0.0%"),
    ...
    equity_bridge = [
        ...
        ("(+) Cash, Cash Equivalents & ST Investments ($M)", "=B10", "$#,##0.00", "B83", False),
        ("Net Debt Adjustment ($M)", "=B82+B83", "$#,##0.00", "B84", False),
    ...
    # Axis ranges centered on Base Case (WACC = 14.96%, g = 3.00%)
    ...
    ws_dcf["A105"] = "Growth Index \\ EBIT Margin"
    ...
    # -------------------------------------------------------------------------
    ws_dcf["A114"] = "TABLE 3: IMPLIED ESTIMATE ($) - EQUITY BETA vs. RISK-FREE RATE (Rf)"
    ...
    # Set column widths for DCF Sheet
    ws_dcf.column_dimensions["A"].width = 46
    ws_dcf.column_dimensions["B"].width = 18
    ws_dcf.column_dimensions["C"].width = 18
    ws_dcf.column_dimensions["D"].width = 18
    ws_dcf.column_dimensions["E"].width = 18
    ws_dcf.column_dimensions["F"].width = 18
    ws_dcf.column_dimensions["G"].width = 18
    ws_dcf.column_dimensions["H"].width = 20
    
    wb.save(output_path)
    print(f"Successfully generated DCF model at: {output_path}")

if __name__ == "__main__":
    build_tsla_dcf_model()
